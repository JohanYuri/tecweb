...
$(document).ready(function(){
    ...
    function listarProductos() {
        $.ajax({
            url: "http://localhost/tecweb/actividades/a09/productoapp/backend/products",
            type: 'GET',
    ...
    $('#search').keyup(function() {
        if($('#search').val()) {
            ...
            $.ajax({
                url: `http://localhost/tecweb/actividades/a09/productoapp/backend/products/${search}`,
                type: 'GET',
                success: function (response) {
                    if(!response.error) {
                        // SE OBTIENE EL OBJETO DE DATOS A PARTIR DE UN STRING JSON
    ...
    $('#product-form').submit(e => {
        ...
        if(!valid){
            ...
            const url = edit === false
                ? 'http://localhost/tecweb/actividades/a09/productoapp/backend/product'
                : `http://localhost/tecweb/actividades/a09/productoapp/backend/product/${postData['id']}`;
    ...
    $(document).on('click', '.product-delete', (e) => {
        if(confirm('¿Realmente deseas eliminar el producto?')) {
            ...
            $.ajax({
                url: `http://localhost/tecweb/actividades/a09/productoapp/backend/product/${id}`,
                type: 'DELETE',
                success: function(response) {
                    $('#product-result').hide();
                    listarProductos();
